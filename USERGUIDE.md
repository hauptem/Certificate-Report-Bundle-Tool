...
Signature files are recognized by their extension (`.sha256`, `.sha384`, `.sha512`, `.p7s`, `.p7m`, `.sig`), and the manifest must be sha256sum, sha384sum, or sha512sum output. Attached and detached signatures both work, with RSA PKCS#1 v1.5, RSA-PSS, or ECDSA on P-256, P-384, or P-521. Co-signed releases are checked against the first signer only, and chain building assumes v3 certificates: an issuer without Basic Constraints is rejected as a non-CA, while issuer keyUsage and extended key usage are not evaluated; the root fingerprint comparison remains the trust decision. Files are read fully into memory for hashing, so multi-gigabyte bundle files may fail in a browser-dependent way.
